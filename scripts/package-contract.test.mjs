...
test('theme has an opt-in reset and all public presets own the same character vocabulary', async () => {
  ...
  assert.match(reset, /^@layer theme, base, components, utilities;$/mu)
  assert.match(reset, /^@import '\.\/preflight\.css' layer\(base\);$/mu)
  const resetBase = cssBlock(reset, '@layer base')
  const resetSupportStart = resetBase.indexOf('\n  * {\n')
  assert.notEqual(resetSupportStart, -1, 'missing universal reset support selector')
  const resetSupport = cssBlock(resetBase.slice(resetSupportStart + 1), '*')
    .replace(/\/\*[\s\S]*?\*\//gu, '')
    .split(';')
