...
| 2026-08-30 | cleanup correction PR #99 | Close run `33287582694`; branch `fix/request-preview-cleanup` | The close run deactivated both deployments but left Pages bytes because `git ls-remote` ran outside the temporary Pages repository. The command and executable sibling-preservation test were corrected; live deletion remains the final PR #99 close-path proof |
...
| RQ-R11 | closed | Preview cleanup reported success while an unscoped `git ls-remote` silently skipped the Pages deletion | PR #99 scopes the remote check to the temporary Pages repository and makes the executable test reject an unscoped call |
