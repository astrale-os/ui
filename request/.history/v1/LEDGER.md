...
| RQ-D12 | accepted | Screenshots are mandatory evidence; live preview uses a replaceable GitHub deployment URL adapter | PR #97 live Pages deployment, exact revision marker, two screenshot links, and browser proof |
| RQ-D13 | accepted | Candidate execution uses isolated credential-free qualification; a fixed base publisher transfers only admitted static bytes without executing them | Three-job secret census, base-controlled programs, artifact admission, and live worker runs `33286794200` and `33287125029` |
| RQ-D14 | accepted | Request PRs never publish packages; existing merge and release owners retain publication | Merged PR #97 followed ordinary CI and merge; request workflows have no package publication authority |
...
| RQ-Q03 | accepted | Which static host provides the smallest isolated, expiring PR preview adapter for this public repo? | Scoped GitHub Pages paths plus transient GitHub deployments; PR #97 proved publication and exposed cleanup defects |
...
| 2026-08-30 | live request issue #87 | Coordinator runs `33279423951`, `33279567570`, `33280688399`, and `33281118369`; worker run `33281140507` | Attempts 1–3 exposed empty accepted-comment IDs despite preceding member comments; after exact repository-permission fallback shipped, attempt 4 captured both comment IDs and consumed the label. Its proposal correctly failed the unrelated UI-source evidence gate because the disposable request was documentation-only |
| 2026-08-30 | live request issue #90 | Coordinator runs `33281408167` and `33282642081`; worker runs `33281430546` and `33282659608`; disposable PR #92; final CI run `33282894531` | A source-backed labeled attempt captured both existing comment IDs and opened one PR; a later maintainer comment plus the same label produced operation `revision`, captured all three IDs, updated the same PR with the requested second named preview, and passed every ordinary PR check. The proof-only PR, branch, and issue were then closed rather than merged |
| 2026-08-30 | request loop implementation PR #95 | Merge `672181c63bd109b34d907e9f5249348803227ebd`; CI `33286326144`; local 122-test request suite; copied-runner screenshot | Issue and PR discussion, exact-one-PR revision dispatch, base-controlled preview planning/capture/admission, revision-keyed Pages evidence, deployments, and cleanup shipped; all remote checks passed |
| 2026-08-30 | live request issue #96 and PR #97 | Worker runs `33286794200` and `33287125029`; revisions `1c18360153f3b6f1d17575af82ef832a7bf25f51` and `16ef4a2e4994102871be7a46129643dc925125f2`; merge `f4b417f9764a71934f332b66952f13dbf78fb847` | Attempt 1 captured the issue comment, opened one PR, published two screenshots and a live named preview. A PR comment plus `ui:ready` produced attempt 2 on the same PR, captured both discussion IDs, changed August 20 to Critical and the statistic from 23 to 22 healthy days, refreshed the same evidence comment/deployment, passed ordinary CI, and merged |
| 2026-08-30 | hosted browser proof | `https://astrale-os.github.io/ui/pr-97/` at both exact revisions | Initial scene was ready with 23 healthy days; revised scene was ready with 22 healthy days, August 20 Critical, and August 21 Healthy. Address, scene, revision meta, accessible per-day labels, and screenshot rendering were observed directly |
| 2026-08-30 | live correction PR #98 | Coordinator run `33287100435`; merge `a0c68c318fe84fba808df48be314ad0efc41215c` | Successful revision initially ended red because GitHub required pull-request write permission to consume `ui:ready`; the exact permission was added and all CI passed |
| 2026-08-30 | cleanup correction PR #99 | Close run `33287582694`; merge `a91ae052650eed64d573b817d817e89ac9550809`; Pages commit `b8753bbb0f1b8baf9619cf5bd56e7fdd93f37dab`; Pages run `33288028424` | The close run deactivated both deployments but initially left Pages bytes because `git ls-remote` ran outside the temporary Pages repository. The corrected command removed exactly the scoped `pr-97` tree, the repository content API reported it absent, Pages redeployed successfully, and the revision URL returned HTTP 404 |
| 2026-08-30 | UI Domain deployment drift | Latest CLI source call to `/:ui.astrale.ai:function.request`; prior `ui.astrale.ai` hostname lookup | Active `bryan` Instance returned Kernel code `3001` and the prior hostname did not resolve. The issue-to-merge proof therefore began with a direct GitHub issue and does not certify current CLI submission |
...
| RQ-R10 | closed | GitHub refused to consume `ui:ready` from a PR with pull-request read permission after a successful revision | PR #98 grants exact pull-request write permission to the coordinator and locks it in the workflow contract |
| RQ-R11 | closed | Preview cleanup reported success while an unscoped `git ls-remote` silently skipped the Pages deletion | PR #99 scopes the remote check to the temporary Pages repository and makes the executable test reject an unscoped call; Pages commit `b8753bbb` plus HTTP 404 prove the corrected live deletion |
