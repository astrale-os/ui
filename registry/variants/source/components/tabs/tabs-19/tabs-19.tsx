  ...
  return (
    <div className='w-full max-w-md'>
      <Tabs defaultValue='explore' orientation='vertical'>
        <TabsList variant='line' className='h-full rounded-none border-r p-0'>
          {tabs.map(tab => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className='data-active:border-primary! rounded-none border! group-data-vertical/tabs:after:-right-px'
            >
              {tab.name}
            </TabsTrigger>
          ))}
