    ...
    <Accordion className='w-full border-0 [&>*>[data-slot="accordion-content"]]:px-0' defaultValue={['item-1']}>
      {items.map((item, index) => (
        <AccordionItem key={index} value={`item-${index + 1}`} className='bg-transparent'>
          <AccordionTrigger className='items-center justify-start gap-4 px-0 **:data-[slot=accordion-trigger-icon]:order-first **:data-[slot=accordion-trigger-icon]:ml-0'>
            <span className='*:[svg]:text-muted-foreground flex items-center gap-4 *:[svg]:size-4 *:[svg]:shrink-0'>
              {item.icon}
