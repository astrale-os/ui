...
const SEMANTIC_KEYS: Record<number, 'noData' | 'critical' | 'degraded' | 'healthy'> = {
  0: 'noData',
  1: 'critical',
  2: 'degraded',
  3: 'healthy',
...
const StatusHeatmapContext = createContext<StatusHeatmapContextType | null>(null)

const useStatusHeatmap = () => {
  const context = use(StatusHeatmapContext)

  if (!context) {
    throw new Error('StatusHeatmap components must be used within a StatusHeatmap')
  }
...
export const StatusHeatmapBlock = ({
  ...
  const { blockSize, blockWidth, blockMargin, blockRadius, labels, colors } = useStatusHeatmap()
...
export type StatusHeatmapBodyProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  hideDateLabels?: boolean
  labelInterval?: number // Show label every N days
  className?: string
  labelClassName?: string
  children: (props: { activity: StatusActivity; dayIndex: number }) => ReactNode
}
    ...
    >
      ...
      >
        ...
        {allActivities.map((activity, dayIndex) => {
          ...
          return <Fragment key={key}>{children({ activity, dayIndex })}</Fragment>
        })}
      </svg>
    </div>
  )
...
export const StatusHeatmapFooter = ({ className, ...props }: StatusHeatmapFooterProps) => (
  <div
...
export type StatusHeatmapStatProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  compute?: (data: StatusActivity[]) => number | string
  label?: string // Template overriding labels.stat. Placeholder: {{value}}
  children?: (result: { value: number | string; data: StatusActivity[] }) => ReactNode
}
...
export const StatusHeatmapStat = ({
  ...
  const value = compute ? compute(data) : data.filter((a) => a.value === healthyValue).length
...
export type StatusHeatmapLegendProps = Omit<HTMLAttributes<HTMLFieldSetElement>, 'children'> & {
  labels?: StatusHeatmapLabels['statuses']
...
export const StatusHeatmapLegend = ({
  ...
  const { labels, statusValues, blockSize, blockWidth, blockRadius, colors } = useStatusHeatmap()
    ...
    >
      {statuses.map((status) =>
        children ? (
          ...
          <div key={`status-${status.value}`} className="flex items-center gap-1">
            ...
            <span className="font-medium text-muted-foreground text-xs">{status.label}</span>
          </div>
        ),
      )}
    </fieldset>
